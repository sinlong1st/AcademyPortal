...
import { ratingTeacher, getMyRating, clearErrors, clearSuccess } from '../actions/ratingActions';
...
class ModalRating extends Component {
  ...
  onSubmit = e => {
    ...
    this.props.ratingTeacher(ratingData, this.props.teacherId, this.props.match.params.id)
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!isEmptyObj(nextProps.rating)) {
      const { my_rating, loading } = nextProps.rating
      this.setState({
        star1: my_rating.star1,
        star2: my_rating.star2,
        star3: my_rating.star3,
        star4: my_rating.star4,
        star5: my_rating.star5,
        text: my_rating.text,
        loading
      })
    }
  ...
  onOpenModal = e => {
    ...
    this.props.getMyRating(this.props.teacherId, this.props.match.params.id);
    this.setState({
      modal: !this.state.modal
    });
  }
  ...
  render() {
    ...
    return (
      <Fragment>
        <Button onClick={this.onOpenModal} color="warning"><i className="fa fa-star"></i> Đánh giá</Button>
        <Modal isOpen={this.state.modal} toggle={this.toggle} className='modal-lg'>
          ...
          <ModalFooter>
            <Button color="warning" onClick={this.onSubmit}>Đánh giá</Button>{' '}
...
export default withRouter(connect(mapStateToProps, { ratingTeacher, getMyRating, clearErrors, clearSuccess })(ModalRating));
